...
def PressureTestRange():
    ...
    presVals  = np.array([-350.0, -50.0, 0.0, 600.0, 1000.0])
...
def GetRefPres():
    ...
    refPres = 0.633156 * float(subprocess.check_output('cat /sys/bus/iio/devices/iio:device0/in_voltage1_raw', shell = True)) - 450.28148 
...
def SetValves(valvePositions):
    #function to the set individual valve positions
...
def PingCentrifugeNode(compressor):
    ...
    nodeRawData = subprocess.check_output(nodeString, shell = True)           #Ping centrifuge Node and save the message
    match       = re.search('(?<=hpPressure: )(.*)', nodeRawData)             #regular expression search for hp sensor counts
...
#gotta love copy/pasting code from stackoverflow...
...
#####################i#############################
#File Setup
generalData = InitializeGeneralDataFile() #setup the data file
...
compressor      = '0'
hpCounts        = 0
creepCheck      = 150.0                            #time letting the cassette sit loaded with nothing happening. want to look at preload creep
